...
inputData = pandas.read_csv('C:\\IIT\\Machine Learning\\Data\\ChicagoCompletedPotHole.csv')
...
for c in range(maxNClusters-1):
   ...
   _thisKmeans = cluster.KMeans(n_clusters = KClusters, random_state = 2022484)
...
result_df = pandas.DataFrame([nClusters, Elbow, Silhouette, Calinski]).transpose()
result_df.columns = ["N Clusters", "Elbow Value", "Silhouette Value", "Calinski-Harabasz Score"]
