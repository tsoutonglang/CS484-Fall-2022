# -*- coding: utf-8 -*-
"""
Created on Sat Oct 22 18:07:43 2022
...
catName = ['CREDIT_SCORE_BAND']
intName = ['BLUEBOOK_1000', 'CUST_LOYALTY', 'MVR_PTS', 'TIF', 'TRAVTIME']
yName = 'CLAIM_FLAG'

nPredictor = len(catName) + len(intName)

cars = pandas.read_csv('C:\\Users\\tsout\\OneDrive\\Desktop\\cs484\\midterm\\policy_2001.csv')
...
for step in range(nPredictor):
    ...
    for X_name in cName:
        ...
        BIC = df0 * numpy.log(n_sample) - 2.0 * llk0
        stepDetail.append([X_name, 'categorical', df1, llk1, devChiSq, devDF, devSig, AIC, BIC])
...
print('======= Step Summary =======')
stepSummary = pandas.DataFrame(stepSummary, columns = ['Predictor', 'Type', 'ModelDF', 'ModelLLK', 'DevChiSq', 'DevDF', 'DevSig', 'AIC', 'BIC'])
print(stepSummary)
